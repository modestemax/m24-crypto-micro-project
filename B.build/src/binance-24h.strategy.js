...
module.exports = class extends Template {
    ...
    assetChanged(asset, newAsset) {
        if (/\/BTC/.test(asset.symbol)) {
            ...
            m24.bidVolume = asset.info.bidQty * asset.info.bidPrice;
            m24.volumeRatio = m24.bidVolume / asset.info.quoteVolume * 100
            ...
            if (change > BREAK_CHANGE) {//faire aumoins 3% 
                if (asset.percentage > 2) //ne pas toucher a ceux qui sont descendant
                    ...
                    if (delta < .5) //if (delta < .5) //se rassurer des petits pas entre les variations
                        if (maxInstantDelta < 1)//pas de hause/chute (pique) brusque
                            if (growingUpSmoothly)//monté progressive
                                if (asset.info.quoteVolume > 40 && bidVolume < 1)//assez bon volume 24H
            ...
            if (change < -1 || asset.percentage > 15 || delta > .5 || maxInstantDelta > 1) {
                this.initAsset(asset, newAsset);
            }
        }
    }
